...
document.addEventListener('keydown', async(e)=>{
  if(keysAllowed && sample.includes(e.key.toLowerCase()) && inputString.innerHTML.length!=6 && !e.repeat){
    ...
    data.forEach((object)=>{
      if(object.result==inputString.innerHTML.toLowerCase() && !solved.includes(string.innerHTML.toLowerCase())){
        correct = true
        ...
        scoreText.animate(
          ...
          object.occupied.forEach((cellNo)=>{
            let blocksArray = getBlockeAtCellNo(cellNo)
    ...
    !correct && new Audio('correct.mp3').play()
    inputString.innerHTML = ''
  }
